"""
Bitget exchange adapter – implements the Exchange interface using CCXT.
"""
import asyncio
import logging
...
class BitgetExchange(Exchange):
        ...
        self.exchange = ccxt.bitget(
            {
                ...
                "sandbox": sandbox,
                "options": self.market_options(settings.EXCHANGE_MARKET_TYPE, "bitget"),
    ...
    def _init_pro(self):
        try:
            self.pro = ccxtpro.bitget(
                {
                    "apiKey": self.credentials.get("apiKey"),
                    "secret": self.credentials.get("secret"),
                    "password": self.credentials.get("passphrase"),
                    "sandbox": self.sandbox,
                    "options": self.market_options(settings.EXCHANGE_MARKET_TYPE, "bitget"),
                }
            )
        except Exception as e:
            logger.warning("Failed to initialise ccxt.pro for Bitget: %s", e)
    ...
    def create_order(self, order: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            logger.error("Order creation failed on Bitget: %s", e)
            raise
    ...
    def cancel_order(self, symbol: str, order_id: str) -> bool:
        try:
            ...
            logger.warning("Order %s not found on Bitget", order_id)
            return False
        except Exception as e:
    ...
    def fetch_open_orders(self, symbol: str = None) -> List[OrderResponse]:
        try:
            ...
            logger.error("Failed to fetch open orders on Bitget: %s", e)
            return []
    ...
    def fetch_positions(self, symbol: str = None) -> List[PositionData]:
        try:
            ...
            logger.error("Failed to fetch positions on Bitget: %s", e)
            return []
    ...
    async def watch_ticker(self, symbol: str, callback=None) -> MarketData:
        if not self.pro:
            self._init_pro()
        if self.pro is None:
            raise RuntimeError("Bitget WebSocket client is unavailable")
    ...
    async def watch_ohlcv(self, symbol: str, timeframe: str, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("WebSocket OHLCV error on Bitget: %s", e)
    ...
    async def watch_order_book(self, symbol: str, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("WebSocket order book error on Bitget: %s", e)
                await asyncio.sleep(1)
    ...
    async def watch_positions(self, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("Position polling error on Bitget: %s", e)
                await asyncio.sleep(5)
    ...
    async def watch_orders(self, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("Order polling error on Bitget: %s", e)
                await asyncio.sleep(2)
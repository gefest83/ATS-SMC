"""
Binance exchange adapter – implements the Exchange interface using CCXT.
Supports both REST and WebSocket (ccxt.pro) for real‑time data.
"""
import asyncio
import logging
...
class BinanceExchange(Exchange):
        ...
        self.exchange = ccxt.binance(
            {
                ...
                "sandbox": sandbox,
                "options": self.market_options(settings.EXCHANGE_MARKET_TYPE, "binance"),
    ...
    def _init_pro(self):
        """
        Initialise the ccxt.pro instance for WebSocket streams.
        """
        try:
            self.pro = ccxtpro.binance(
                {
            ...
            logger.warning("Failed to initialise ccxt.pro for Binance: %s", e)
    ...
    def create_order(self, order: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            logger.error("Order creation failed on Binance: %s", e)
            raise
    ...
    def cancel_order(self, symbol: str, order_id: str) -> bool:
        try:
            ...
            logger.warning("Order %s not found on Binance", order_id)
            return False
        except Exception as e:
    ...
    def fetch_open_orders(self, symbol: str = None) -> List[OrderResponse]:
        try:
            ...
            logger.error("Failed to fetch open orders on Binance: %s", e)
            return []
    ...
    def fetch_positions(self, symbol: str = None) -> List[PositionData]:
        try:
            ...
            logger.error("Failed to fetch positions on Binance: %s", e)
            return []
    ...
    # -------------------------------------------------------------------- #
    # WebSocket implementations (ccxt.pro)
    # -------------------------------------------------------------------- #
    async def watch_ticker(self, symbol: str, callback=None) -> MarketData:
        if not self.pro:
            self._init_pro()
        if self.pro is None:
            raise RuntimeError("Binance WebSocket client is unavailable")
    ...
    async def watch_ohlcv(self, symbol: str, timeframe: str, callback=None):
        ...
        while True:
            try:
                ...
                # Simple conversion – in production you'd map to a dedicated OHLCV model
                await self._dispatch_callback(callback, ohlcv)
            except Exception as e:
                logger.error("WebSocket OHLCV error on Binance: %s", e)
    ...
    async def watch_order_book(self, symbol: str, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("WebSocket order book error on Binance: %s", e)
                await asyncio.sleep(1)

    async def watch_positions(self, callback=None):
        if not self.pro:
            self._init_pro()
        # Binance does not expose a direct position stream for spot; use REST polling
        while True:
            try:
                ...
                logger.error("Position polling error on Binance: %s", e)
                await asyncio.sleep(5)
    ...
    async def watch_orders(self, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("Order polling error on Binance: %s", e)
                await asyncio.sleep(2)
"""
Gate.io exchange adapter – implements the Exchange interface using CCXT.
"""
import asyncio
import logging
...
class GateIOExchange(Exchange):
        ...
        self.exchange = ccxt.gateio(
            {
                ...
                "sandbox": sandbox,
                "options": self.market_options(settings.EXCHANGE_MARKET_TYPE, "gateio"),
    ...
    def _init_pro(self):
        try:
            ...
            logger.warning("Failed to initialise ccxt.pro for Gate.io: %s", e)
    ...
    def create_order(self, order: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            logger.error("Order creation failed on Gate.io: %s", e)
            raise
    ...
    def cancel_order(self, symbol: str, order_id: str) -> bool:
        try:
            ...
            logger.warning("Order %s not found on Gate.io", order_id)
            return False
        except Exception as e:
    ...
    def fetch_open_orders(self, symbol: str = None) -> List[OrderResponse]:
        try:
            ...
            logger.error("Failed to fetch open orders on Gate.io: %s", e)
            return []
    ...
    def fetch_positions(self, symbol: str = None) -> List[PositionData]:
        try:
            ...
            logger.error("Failed to fetch positions on Gate.io: %s", e)
            return []
    ...
    async def watch_ticker(self, symbol: str, callback=None) -> MarketData:
        if not self.pro:
            self._init_pro()
        if self.pro is None:
            raise RuntimeError("Gate.io WebSocket client is unavailable")
    ...
    async def watch_ohlcv(self, symbol: str, timeframe: str, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("WebSocket OHLCV error on Gate.io: %s", e)
    ...
    async def watch_order_book(self, symbol: str, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("WebSocket order book error on Gate.io: %s", e)
                await asyncio.sleep(1)
    ...
    async def watch_positions(self, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("Position polling error on Gate.io: %s", e)
                await asyncio.sleep(5)
    ...
    async def watch_orders(self, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("Order polling error on Gate.io: %s", e)
                await asyncio.sleep(2)
"""
Bybit exchange adapter – implements the Exchange interface using CCXT.
"""
import asyncio
import logging
...
class BybitExchange(Exchange):
        ...
        self.exchange = ccxt.bybit(
            {
                ...
                "sandbox": sandbox,
                "options": self.market_options(settings.EXCHANGE_MARKET_TYPE, "bybit"),
    ...
    def _init_pro(self):
        try:
            ...
            logger.warning("Failed to initialise ccxt.pro for Bybit: %s", e)

    # REST implementations (similar to Binance, adapted for Bybit)
    ...
    def create_order(self, order: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            logger.error("Order creation failed on Bybit: %s", e)
            raise
    ...
    def cancel_order(self, symbol: str, order_id: str) -> bool:
        try:
            ...
            logger.warning("Order %s not found on Bybit", order_id)
            return False
        except Exception as e:
    ...
    def fetch_open_orders(self, symbol: str = None) -> List[OrderResponse]:
        try:
            ...
            logger.error("Failed to fetch open orders on Bybit: %s", e)
            return []
    ...
    def fetch_positions(self, symbol: str = None) -> List[PositionData]:
        try:
            ...
            logger.error("Failed to fetch positions on Bybit: %s", e)
            return []
    ...
    # WebSocket methods (similar to Binance)
    async def watch_ticker(self, symbol: str, callback=None) -> MarketData:
        if not self.pro:
            self._init_pro()
        if self.pro is None:
            raise RuntimeError("Bybit WebSocket client is unavailable")
    ...
    async def watch_ohlcv(self, symbol: str, timeframe: str, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("WebSocket OHLCV error on Bybit: %s", e)
    ...
    async def watch_order_book(self, symbol: str, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("WebSocket order book error on Bybit: %s", e)
                await asyncio.sleep(1)
    ...
    async def watch_positions(self, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("Position polling error on Bybit: %s", e)
                await asyncio.sleep(5)
    ...
    async def watch_orders(self, callback=None):
        ...
        while True:
            try:
                ...
                logger.error("Order polling error on Bybit: %s", e)
                await asyncio.sleep(2)